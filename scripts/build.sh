...
if [ "$1" == "main" ]; then
    if [ "$2" == "--release" ]; then
        ...
        cmake -S . -B build/main/release -DCMAKE_C_COMPILER="mpicc" -DCMAKE_CXX_COMPILER="mpicxx" -DCMAKE_CXX_FLAGS="-O3";
        cmake --build build/main/release --target main -j12;
    elif [ "$2" == "--debug" ]; then
        if [ ! -d build/main/debug ]; then mkdir build/main/debug; fi
        cmake -S . -B build/main/debug -DCMAKE_C_COMPILER="mpicc" -DCMAKE_CXX_COMPILER="mpicxx" -DCMAKE_CXX_FLAGS="-O0 -g";
        cmake --build build/main/debug --target main -j12;
    elif [ "$2" == "--profile" ]; then
        if [ ! -d build/main/profile ]; then mkdir build/main/profile; fi
        cmake -S . -B build/main/profile -DCMAKE_C_COMPILER="mpicc" -DCMAKE_CXX_COMPILER="mpicxx" -DCMAKE_CXX_FLAGS="-O3 -pg";
