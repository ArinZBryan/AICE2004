...
public:
	...
	void operator()(const tbb::blocked_range<size_t>& r) {
		for (auto i = r.begin(); i != r.end(); i++) {
			...
			Network::TrainResult tr = Network::train(sample_data, label_one_hot, weights, biases);
	...
	ParallelTrainBody(ParallelTrainBody& other, tbb::split) : samples(other.samples), weights(other.weights), biases(other.biases), _hidden_size(other._hidden_size) {
		result = {
	...
	ParallelTrainBody(const std::vector<Sample>& samples, const std::vector<Matrix>& weights, const std::vector<Vector>& biases, size_t hidden_size)
	 : samples(samples), weights(weights), biases(biases), _hidden_size(hidden_size), 
		result{
...
std::vector<float> train_model_mini_batch_parallel(Network &model, const std::vector<Sample>& data, const TrainConfig &config) {
	...
	for (unsigned int epoch = 0; epoch < config.epochs; ++epoch) {
		...
		for (size_t batch = 0; batch < batches; batch++) {
			...
			ParallelTrainBody ptb(data, model.get_weights(), model.get_bias(), config.hidden_size);
