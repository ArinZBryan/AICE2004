...
Matrix::Matrix(size_t nrows, size_t ncols, const number& fill) : _data(nrows * ncols, fill), _rows(nrows), _cols(ncols) {};
Matrix::Matrix(std::initializer_list<std::initializer_list<number>> init) {
    std::vector<std::vector<number>> jagged;
    for (auto inner : init) {
        jagged.emplace_back(std::vector<number>(inner));
    }
    _rows = jagged.size();
    size_t columns = jagged[0].size();
    _cols = columns;
    _data = std::vector<number>(jagged.size() * jagged[0].size());
...
Matrix& Matrix::operator=(const Matrix& other) {
    _data = std::vector<number>((const std::vector<number>&)other._data);
    _rows = other._rows;
    _cols = other._cols;
    return *this;
};
...
std::vector<std::vector<float>> Matrix::toJaggedArrays() const {
    ...
    for (size_t i = 0; i < _rows; i++) for (size_t j = 0; j < _cols; j++) {
        jagged[i][j] = static_cast<float>((*this)(i, j));
    }
    return jagged;
}